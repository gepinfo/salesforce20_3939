...
export class salesforceentityController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/salesforceentity/:id', this.GpDelete);
this.router.get('/salesforceentity/get/search', this.GpSearch);
this.router.put('/salesforceentity/get/update', this.GpSearchForUpdate);
this.router.put('/salesforceentity', this.GpUpdate);
this.router.get('/salesforceentity/:id', this.GpGetEntityById);
this.router.get('/salesforceentity', this.GpGetAllValues);
this.router.post('/salesforceentity', this.GpCreate);
this.router.get('/salesforceentity/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
    }
...
public GpDelete(req: Request, res: Response) {
        ...
        new ApiAdapter().delete(Constant.CREATEACCOUNTURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.CREATEACCOUNTURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpSearchForUpdate(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into salesforceentityController.ts: GpSearchForUpdate');
        new ApiAdapter().put(Constant.CREATEACCOUNTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from salesforceentityController.ts: GpSearchForUpdate');
        }).catch(err => {
            res.send(err);
        });
    }
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.CREATEACCOUNTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
